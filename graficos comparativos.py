...
# CONFIGURAÇÃO: Defina aqui o caminho da pasta com os arquivos CSV
PASTA_CSV = "D:\Trabalhos UTFPR\Oficinas"  # Altere para o caminho da sua pasta
# Exemplos alternativos:
# PASTA_CSV = "C:/Users/SeuUsuario/Desktop/dados_experimento"
# PASTA_CSV = "../resultados"
# PASTA_CSV = "C:/Projetos/agilidade_dados"
...
class GraficoAgilidade:
        ...
        for arquivo in self.arquivos_selecionados:
            try:
                ...
                if 'Tempo_Reacao(s)' in df.columns and 'Numero_Clique' in df.columns:
                    # Garantir que temos exatamente 30 cliques
                    dados_30 = df.head(30).copy()
                    
                    if len(dados_30) == 30:
                        self.dados[nome_arquivo] = dados_30
                    else:
                        print(f"Aviso: {nome_arquivo} tem apenas {len(dados_30)} cliques")
                else:
    ...
    def gerar_grafico(self):
        ...
        for i, (nome_arquivo, df) in enumerate(self.dados.items()):
            ...
            # Garantir que temos 30 pontos
            if len(tempos) < 30:
                tempos = np.pad(tempos, (0, 30 - len(tempos)), 'constant', constant_values=np.nan)
        ...
        if len(todos_tempos) > 1 and self.var_media.get():
            media_geral = np.nanmean(todos_tempos, axis=0)
            self.ax.plot(range(1, 31), media_geral, color='red', linewidth=3, 
                       label='Média Geral', linestyle='--', marker='o', markersize=6)
            
            # Adicionar linha de tendência se solicitado
            if self.var_tendencia.get():
                x_trend = range(1, 31)
                valid_mask = ~np.isnan(media_geral)
                if np.sum(valid_mask) > 1:
                    z = np.polyfit(np.array(x_trend)[valid_mask], media_geral[valid_mask], 1)
                    p = np.poly1d(z)
                    self.ax.plot(x_trend, p(x_trend), "r--", alpha=0.8, linewidth=2, 
        ...
        self.ax.set_title('Tempo de Reação por Clique - Teste de 30 Cliques', 
                         fontsize=14, fontweight='bold', pad=20)
        
        self.ax.set_xticks(range(1, 31))
